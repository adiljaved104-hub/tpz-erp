...
class ViewQuotation extends ViewRecord
{
    ...
    protected function getHeaderActions(): array
    {
        ...
        return [
            ...
            Action::make('reject')->color('danger')->schema([Textarea::make('reason')->required()])->visible(fn () => QuotationResource::allowed(QuotationPermission::Reject, $this->record) && $status === QuotationStatus::Sent)->action(fn (array $data) => app(QuotationService::class)->transition($this->record, QuotationStatus::Rejected, auth()->user(), $data['reason'])),
            Action::make('cancel')->color('danger')->schema([Textarea::make('reason')->required()])->visible(fn () => QuotationResource::allowed(QuotationPermission::Cancel, $this->record) && in_array($status, [QuotationStatus::Draft, QuotationStatus::Sent, QuotationStatus::Accepted], true))->action(fn (array $data) => app(QuotationService::class)->transition($this->record, QuotationStatus::Cancelled, auth()->user(), $data['reason'])),
            Action::make('convertOrder')->label('Convert to Order')->icon('heroicon-o-shopping-cart')->schema([
                Select::make('warehouse_id')->label('Fulfilment Warehouse')->default(fn () => $this->record->warehouse_id)->disabled(fn () => $this->record->warehouse_id !== null)->dehydrated()->options(fn () => app(OrderFulfillmentLocationService::class)->options(null))->required(),
                TextInput::make('customer_phone')->label('Customer Phone')->tel()->default(fn () => $this->record->customer_phone)->required()->maxLength(40)->regex('/^\+?[0-9][0-9\s().-]{5,39}$/'),
            ])->visible(fn () => QuotationResource::allowed(QuotationPermission::ConvertOrder, $this->record) && $status === QuotationStatus::Accepted && ! $this->record->order_id)->action(function (array $data): void {
                $o = app(QuotationConversionService::class)->toOrder($this->record, (int) $data['warehouse_id'], auth()->user(), customerPhone: $data['customer_phone']);
