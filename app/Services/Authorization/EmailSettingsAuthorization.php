...
class EmailSettingsAuthorization
{
    public function __construct(private readonly EmployeePermissionOverrideResolver $overrides) {}

    public function roleDefault(User $user, EmailSettingsPermission $permission): bool
    {
        return in_array($user->employee?->role, [EmployeeRole::Owner, EmployeeRole::Admin], true);
    }

    public function allows(User $user, EmailSettingsPermission $permission): bool
    {
    ...
    public function authorize(User $user, EmailSettingsPermission $permission): void
    {
