 ...
 WHERE sti.stock_transfer_id = stock_transfers.id AND NOT EXISTS (
  ...
  WHERE transfer_ra.employee_id = ? AND transfer_ra.status = 'active' AND transfer_ra.ended_at IS NULL
   ...
   AND (
    ...
    OR EXISTS (SELECT 1 FROM responsibility_assignment_warehouses rw WHERE rw.assignment_id = transfer_ra.id AND rw.warehouse_id = stock_transfers.source_warehouse_id)
    OR (EXISTS (SELECT 1 FROM responsibility_assignment_platforms rpo WHERE rpo.assignment_id = transfer_ra.id)
     ...
     AND NOT EXISTS (SELECT 1 FROM responsibility_assignment_categories rcn WHERE rcn.assignment_id = transfer_ra.id)
     AND NOT EXISTS (SELECT 1 FROM inventory_responsibility_quantities rqn WHERE rqn.assignment_id = transfer_ra.id)
     AND NOT EXISTS (SELECT 1 FROM responsibility_assignment_warehouses rwn WHERE rwn.assignment_id = transfer_ra.id))
   )
 )
)
SQL, [$employeeId]);
