                    ...
                    <table class="w-full min-w-max divide-y divide-gray-200 text-sm dark:divide-white/10">
                        <thead class="bg-gray-50 text-left text-xs font-semibold uppercase tracking-wide text-gray-600 dark:bg-white/5 dark:text-gray-300">
                            <tr>
                                ...
                                @if (in_array('reserved', $visibleColumns, true))<th class="min-w-24 px-4 py-3 text-center">Physical Reserved</th>@endif
                                @if (in_array('my_reserved', $visibleColumns, true))<th class="min-w-24 px-4 py-3 text-center">My Reserved</th>@endif
                                @if (in_array('other_allocated', $visibleColumns, true))<th class="min-w-28 px-4 py-3 text-center">Other Allocated</th>@endif
                                @if (in_array('system_unallocated', $visibleColumns, true))<th class="min-w-32 px-4 py-3 text-center">System / Unallocated</th>@endif
                                ...
                                @if (in_array('usable_now', $visibleColumns, true))<th class="min-w-24 px-4 py-3 text-center">My Available</th>@endif
                        ...
                        @foreach ($inventoryRows as $row)
                            <tbody class="border-t border-gray-100 first:border-t-0 dark:border-white/5" x-data="{ detailsOpen: false }">
                                <tr class="align-top" data-inventory-row>
                                    ...
                                    @if (in_array('my_reserved', $visibleColumns, true))<td class="whitespace-nowrap px-4 py-3 text-center">{{ $row->my_reserved }}</td>@endif
                                    @if (in_array('other_allocated', $visibleColumns, true))<td class="whitespace-nowrap px-4 py-3 text-center">{{ $row->other_allocated }}</td>@endif
                                    @if (in_array('system_unallocated', $visibleColumns, true))<td class="whitespace-nowrap px-4 py-3 text-center">{{ $row->system_unallocated }}</td>@endif
                ...
                <div class="tpz-my-inventory-mobile space-y-3">
                    @foreach ($inventoryRows as $row)
                        <article class="min-w-0 rounded-lg border border-gray-200 p-4 dark:border-white/10">
                            ...
                            <dl class="mt-3 grid grid-cols-2 gap-3 text-sm">@if (in_array('brand', $visibleColumns, true))<div><dt class="text-xs text-gray-500">Brand</dt><dd>{{ $row->brand ?: '—' }}</dd></div>@endif @if (in_array('location', $visibleColumns, true))<div><dt class="text-xs text-gray-500">Location</dt><dd class="break-words">{{ $row->warehouse ?: 'No balance' }}</dd></div>@endif @if (in_array('physical_sellable', $visibleColumns, true))<div><dt class="text-xs text-gray-500">Physical Sellable</dt><dd>{{ $row->sellable }}</dd></div>@endif @if (in_array('allocation_remaining', $visibleColumns, true))<div><dt class="text-xs text-gray-500">Quantity Responsibility Remaining</dt><dd>{{ $row->is_quantity_limited ? $row->remaining_allocation.' of '.$row->assigned_quantity : 'Not capped' }}</dd></div>@endif @if (in_array('usable_now', $visibleColumns, true))<div><dt class="text-xs text-gray-500">My Available</dt><dd class="font-semibold">{{ $row->employee_usable }}</dd></div>@endif @if (in_array('reserved', $visibleColumns, true))<div><dt class="text-xs text-gray-500">Physical Reserved</dt><dd>{{ $row->reserved }}</dd></div>@endif @if (in_array('my_reserved', $visibleColumns, true))<div><dt class="text-xs text-gray-500">My Reserved</dt><dd>{{ $row->my_reserved }}</dd></div>@endif @if (in_array('other_allocated', $visibleColumns, true))<div><dt class="text-xs text-gray-500">Other Allocated</dt><dd>{{ $row->other_allocated }}</dd></div>@endif @if (in_array('system_unallocated', $visibleColumns, true))<div><dt class="text-xs text-gray-500">System / Unallocated</dt><dd>{{ $row->system_unallocated }}</dd></div>@endif</dl>
