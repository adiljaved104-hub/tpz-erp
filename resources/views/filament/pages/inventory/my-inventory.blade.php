        ...
        <x-filament::section heading="My Products" description="Search and filter only the inventory available through your active responsibilities.">
            ...
            @else
                <div class="hidden overflow-x-auto rounded-lg border border-gray-200 md:block dark:border-white/10">
                    <table class="w-full min-w-[1200px] table-fixed divide-y divide-gray-200 text-sm dark:divide-white/10">
                        <thead class="bg-gray-50 text-left text-xs font-semibold uppercase tracking-wide text-gray-600 dark:bg-white/5 dark:text-gray-300">
                            <tr><th class="w-64 px-3 py-3">Product</th><th class="w-28 px-3 py-3">Brand</th><th class="w-36 px-3 py-3">Platform</th><th class="w-36 px-3 py-3">Location</th><th class="w-28 px-3 py-3 text-center">Physical Sellable</th><th class="w-24 px-3 py-3 text-center">Reserved</th><th class="w-32 px-3 py-3 text-center">Allocation Remaining</th><th class="w-24 px-3 py-3 text-center">Usable Now</th><th class="w-32 px-3 py-3">Status</th><th class="px-3 py-3">Visible Because</th></tr>
                        </thead>
                        <tbody class="divide-y divide-gray-100 dark:divide-white/5">
                            @foreach ($inventoryRows as $row)
                                <tr class="align-top">
                                    ...
                                    <td class="px-3 py-3 text-center">{{ $row->sellable }}</td>
                ...
                <div class="space-y-3 md:hidden">
                    @foreach ($inventoryRows as $row)
                        <article class="min-w-0 rounded-lg border border-gray-200 p-4 dark:border-white/10">
                            ...
                            <dl class="mt-3 grid grid-cols-2 gap-3 text-sm"><div><dt class="text-xs text-gray-500">Brand</dt><dd>{{ $row->brand ?: '—' }}</dd></div><div><dt class="text-xs text-gray-500">Location</dt><dd>{{ $row->warehouse ?: 'No balance' }}</dd></div><div><dt class="text-xs text-gray-500">Physical Sellable</dt><dd>{{ $row->sellable }}</dd></div><div><dt class="text-xs text-gray-500">Allocation Remaining</dt><dd>{{ $row->is_quantity_limited ? $row->remaining_allocation.' of '.$row->assigned_quantity : 'Shared' }}</dd></div><div><dt class="text-xs text-gray-500">Usable Now</dt><dd class="font-semibold">{{ $row->employee_usable }}</dd></div><div><dt class="text-xs text-gray-500">Reserved</dt><dd>{{ $row->reserved }}</dd></div></dl>
